...
import { X, Loader2, AlertCircle, CheckCircle, ChevronRight } from 'lucide-react';
...
export default function SubmitBourbonModal({ onClose, onSubmit, onSelectExisting, existingBourbons }: SubmitBourbonModalProps) {
  ...
  const handleInputSubmit = async (e: React.FormEvent) => {
    ...
    // Fuzzy match check
    const normalizedInput = normalizeBourbonName(inputName);
    const matches = existingBourbons.filter(b => {
      ...
      // Allow for some typo tolerance relative to string length
  ...
  const generateData = async () => {
    ...
    setError(null);
    try {
      const data = await generateBourbonData(`${inputName} ${inputDetails}`);
